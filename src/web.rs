...
async fn index() -> impl IntoResponse {
    (StatusCode::OK, [("content-type", "text/html; charset=utf-8")], include_str!("../web/index.html"))
}
...
async fn search_entries(Query(query): Query<SearchQuery>) -> impl IntoResponse {
    ...
    match storage::load() {
        Ok(store) => Json(store.entries.into_iter().filter(|e| search::matches(e, &q)).collect::<Vec<_>>()).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()).into_response(),
    }
}
